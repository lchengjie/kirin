...
export const hasPermission = (roles:  string[], route: RouteRecordRaw): boolean => {
  if (route.meta && route.meta.roles) {
    return roles.some(role => {
      ...
      if (metaRoles && Array.isArray(metaRoles)) {
...
export const generateDynamicRoutesv2 = (roles: string[]) => {
  const allRoute = [...routeModules, ...basic];
  // 通过role过滤
  const accessedRoutes = filterAsyncRoutes(allRoute, roles)
  rootRoute.children = accessedRoutes;
