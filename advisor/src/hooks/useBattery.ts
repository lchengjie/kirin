...
export const useBattery = () => {
  ...
  onMounted(async () => {
    ...
    // 电池充电状态更新时被调用
    BatteryManager.onchargingchange = (event: Event) => {
      updateBattery(event.target);
      console.log(event.target, '电池充电状态改变了');
    };
    // 电池充电时间更新时被调用
    BatteryManager.onchargingtimechange = (event: Event) => {
      updateBattery(event.target);
      console.log(event.target, '电池充电了');
    };
    // 电池断开充电时间更新时被调用
    BatteryManager.ondischargingtimechange = (event: Event) => {
      updateBattery(event.target);
      console.log(event.target, '电池断开充电了');
    };
    // 电池电量更新时被调用
    BatteryManager.onlevelchange = (event: Event) => {
      updateBattery(event.target);
      console.log(event.target, '电量更新了');
    };
